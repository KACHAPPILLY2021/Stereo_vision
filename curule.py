...
######################### Function to estimate correct camera pose
# def correct_pose( C1, R1 , C2 , R2 , C3 , R3 , C4 , R4 , x  , y , K):
#  Taking point form image
#     h = np.array([[x] , [y] , [1]])
#     K_inv = np.linalg.inv(K)
## Converting into camera coordinate
#     camera_coordinate = np.matmul(K_inv , h)
#     camera_coordinate = np.concatenate((camera_coordinate, 1) , axis =0)


#     left = np.matmul(K, R1)
#     right = np.concatenate((np.identity(3) , -C1), axis = 1)
## Determine projection matrix
#     P = np.matmul(left , right)

#     X = np.matmul(P , camera_coordinate)

## TAking third row of rotation matrix and multilying with (coordinate - translation)
#     ans = R3 @(X-C)

## the answer(ans) which gives positive value is considered as correct configuration of camera
...
def disparity(img1_rectified , img2_rectified):
    ...
    # window traversing through image, with window size = 10X10 and stride = 1 pixel
    for h in range( img1_rectified.shape[0]-9) :
        for w in range( img1_rectified.shape[1]-9  ) :
            window_left = np.array(img2_rectified[h:h+10 , w:w+10])
            # 1D array to store SSD of each window 
            ssd_store = np.zeros((1 , 60 )) 
            index_ssd = 0
            ...
            for j in range( -30 , 30 ) :
                ...
                if  w>30 and (j+w)<951:
                    window_right = np.array(img1_rectified[h:h+10 , j+w:j+w+10])
                    ...
                    window_right = np.array(img1_rectified[h:h+10 , j+30:j+40])
                    result1 = np.subtract(window_left ,window_right) 
                    result1_sum = np.sum(np.square(result1))
                    ssd_store[0 , index_ssd] = result1_sum
            ...
            disparity[h , w] = min_ssd_index -30

# making all disparity values positive
    disparity = disparity+np.abs(np.min(disparity)) 

    # for depth estimation to avoid '0' value in disparity
    depth_disparity = disparity + 1
    return np.uint8( (disparity/np.max(disparity))*255) ,depth_disparity


# Reading and resizing frames
img_A = cv2.imread("curule/im0.png", cv2.IMREAD_COLOR)
resized_A = resize(img_A , 0.5 , 0.5)

img_B = cv2.imread("curule/im1.png", cv2.IMREAD_COLOR)
...
list_kpA = []
list_kpB = []
# getting coordinates from keypoints
...
############ testing F matrix condition  ############
# xt = np.array([602 , 131 , 1])
# print(np.shape(xt))
# x = np.array([662 , 131 , 1])
# # x = x.transpose()
# # print(np.shape(x))

# left = np.matmul(xt , F_matrix)
# answer = np.matmul(left , x.transpose())
