...
def disparity(img1_rectified , img2_rectified):
    ...
    # window traversing through image, with window size = 20X20 and stride = 1 pixel
    for h in range( img1_rectified.shape[0]-19) :
        for w in range( img1_rectified.shape[1]-19  ) :
            window_left = np.array(img2_rectified[h:h+20 , w:w+20])
            ...
            for j in range( -30 , 30 ) :
                ...
                if  w>30 and (j+w)<941:
                    window_right = np.array(img1_rectified[h:h+20 , j+w:j+w+20])
                    ...
                    window_right = np.array(img1_rectified[h:h+20 , j+30:j+50])
                    result1 = np.subtract(window_left ,window_right) 
                    result1_sum = np.sum(np.square(result1))
                    ssd_store[0 , index_ssd] = result1_sum
...
img_A = cv2.imread("pendulum/im0.png", cv2.IMREAD_COLOR)

resized_A = resize(img_A , 0.5 , 0.5)


img_B = cv2.imread("pendulum/im1.png", cv2.IMREAD_COLOR)
